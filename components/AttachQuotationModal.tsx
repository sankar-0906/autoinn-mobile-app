...
export default function AttachQuotationModal({
  ...
  const resetAndLoad = () => {
    // Don't reset if we're in the middle of a search
    if (searching || searchQuery.trim()) {
      console.log('🔍 Skipping resetAndLoad - search is active');
      return;
    }
  ...
  const loadPage = async (page: number) => {
    // Don't load if we have an active search query
    if (loadedPages.has(page) || !hasMoreRef.current || searchQuery.trim()) {
      console.log('🔍 Skipping loadPage - search is active or page already loaded');
      return;
    }
    ...
    try {
      ...
      if (response.data.code === 200 && response.data.response.code === 200) {
        ...
        // Only update filteredQuotations if there's no active search
        if (!searchQuery.trim()) {
          ...
          console.log('🔍 Skipping filteredQuotations update - search is active');
        }
  ...
  // Enhanced search with debounce - supports Quotation ID search only
  const handleSearch = useCallback((query: string) => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      if (!query.trim()) {
        ...
        console.log('🔍 Search cleared, resetting to show all quotations');
        setSearching(false);
        
        // Reset filtered quotations to show all loaded quotations
        setFilteredQuotations(quotations);
        
        // Only reset and reload if no quotations are loaded
      ...
      // Enhanced search locally in already loaded quotations - by Quotation ID
      const searchLower = query.toLowerCase().trim();
      
      console.log('🔍 Debug - Available quotations count:', quotations.length);
      console.log('🔍 Debug - First few quotations:', quotations.slice(0, 3).map(q => q.quotationId));
      
      // Enhanced matching: exact match, starts with, contains, and partial digit matching
      const results = quotations.filter(item => {
        const quotationId = item.quotationId?.toLowerCase() || '';
        
        // Exact match
        if (quotationId === searchLower) return true;
        
        // Starts with
        if (quotationId.startsWith(searchLower)) return true;
        
        // Contains
        if (quotationId.includes(searchLower)) return true;
        
        // Enhanced: Match last digits (if search contains only digits and is shorter)
        if (/^\d+$/.test(searchLower) && searchLower.length <= 6) {
          // Extract only digits from quotation ID
          const quotationDigits = quotationId.replace(/\D/g, '');
          // Check if search matches the last N digits
          if (quotationDigits.endsWith(searchLower)) return true;
        }
      ...
      console.log('🔍 Debug - Search results:', results.slice(0, 3).map(q => q.quotationId));

      // Update filtered quotations with search results
      setFilteredQuotations(results);
      setSearching(false);
      
      // Show search feedback
      if (query.trim().length > 0) {
        const searchType = /^\d+$/.test(query.trim()) && query.trim().length <= 6 
          ? 'last digits' 
          : 'quotation ID';
        console.log(`🔍 Search by ${searchType}: "${query}" → Found ${results.length} results`);
      }
    }, 300); // 300ms debounce
  }, [quotations]);
        ...
        <View className="bg-white px-4 py-3 border-b border-gray-200">
          <View className="flex-row items-center bg-gray-100 rounded-lg px-3 py-2">
            ...
            <TextInput
              ...
              placeholder="Search by Quotation ID or last digits..."
              value={searchQuery}
            ...
            <View className="flex-row items-center mt-2">
              ...
              <Text className="text-xs text-gray-400">
                {(() => {
                    ...
                    const isLastDigits = isDigitsOnly && query.length <= 6;
                    
                    if (isLastDigits) {
                      return `Searching by last digits... Found ${filteredQuotations.length} quotations`;
                    } else {
                      return `Searching by quotation ID... Found ${filteredQuotations.length} quotations`;
                    }
                  })()
                }
              </Text>
              ...
              {__DEV__ && (
                <View className="bg-yellow-50 p-2 border-b border-yellow-200">
                  <Text className="text-xs text-yellow-800">
                    Debug: Total={quotations.length}, Filtered={filteredQuotations.length}, Searching={searching ? 'true' : 'false'}
                  </Text>
                  <Text className="text-xs text-yellow-800">
                    Query: "{searchQuery}"
                  </Text>
                  ...
                  <View className="flex-1 items-center justify-center py-12">
                    ...
                    {searchQuery && (
                      <Text className="text-gray-400 text-xs mt-2 text-center px-8">
                        Try a different Quotation ID or check if quotations are loaded
                      </Text>
                    )}
                    {!searchQuery && quotations.length === 0 && !loading && (
                      <Text className="text-gray-400 text-xs mt-2 text-center px-8">
                        No quotations loaded. Try refreshing.
                      </Text>
                    )}
                  </View>
                }
